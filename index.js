...
app.get('/:id', dataPull, function(req, res) {
    ...
    for (let i = 0; i < back + 1; i++) {
        const currPoint = makePoint(req.params.id, req.data.at(-(i + 1)).timestamp, req.data.at(-(i + 1)).avgHighPrice, averages[i], devs[i]);
        points[i] = currPoint;
        console.log(currPoint);
        string = string + 'Average value at time ' + (i + 1) + ' is ' + averages.at(i);
        string = string + '<br/>';
    }
    db.collection('points').insertMany(points);
    //
    res.send(string);
});
